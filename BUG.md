...
**期望**：折叠/展开按钮无多余边框线；折叠侧栏不残留右边框细线；按钮悬浮在空闲页录制主面板顶部，按钮下边缘与主面板上边缘重合，按钮右边缘与主面板右边缘对齐。
...
3. 将 `RecordingSidebar` 挂到空闲页录制面板的 `relative` 容器内，toggle 改为 `absolute -top-8 right-0`，不再根据侧栏展开宽度移动。
4. 新增前端回归测试覆盖折叠状态无残留边框、展开状态无左侧边缘线、toggle 无常态边框/阴影、toggle 在展开/折叠时都锚定主面板右上角。
...
29. toggle 按钮的锚点不应跟随抽屉宽度切换；如果需要对齐主操作面板，应把按钮挂到主面板的定位容器内。
